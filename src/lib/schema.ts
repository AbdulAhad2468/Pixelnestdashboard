export async function ensureSchema(sql: any) {
  ...
  // Channel messages
  await sql`CREATE TABLE IF NOT EXISTS channel_messages (
    id VARCHAR(255) PRIMARY KEY,
  ...
  // Private messages
  await sql`CREATE TABLE IF NOT EXISTS private_messages (
    id VARCHAR(255) PRIMARY KEY,
  ...
  await sql`INSERT INTO boards (id, name, created_at)
    VALUES ('1', 'Sprint Board', '2026-07-20T00:00:00.000Z')
    ON CONFLICT (id) DO NOTHING`;

  await sql`INSERT INTO columns (id, board_id, title)
    VALUES
      ('todo', '1', 'To Do'),
  ...
  await sql`INSERT INTO channels (id, name, created_at)
    VALUES
